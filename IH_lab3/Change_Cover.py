...
def Wa_ref_cal(password,msg):
    ...
    for i in range(8):
        ...
        #根据两个状态计算最后的结果
        ref = generate_ref(password+":"+password_state+str(i),(512*512))
        Wm_i.append(ref)
        state = next_state
...
#生成当前的reference集合，大小暂时设定为512*512，之后会随着实际进行修改
    ...
    for f in files:
        ...
        result = ""
        zlclist = decode(Cover_w,ref)
        zlclist_no_watermark = decode(Cover,ref)
